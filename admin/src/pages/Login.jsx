        ...
        <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
            <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border border-[#EADBCE] rounded-2xl text-[#5E5E5E] text-sm shadow-lg bg-white'>
                ...
                <div className='w-full'>
                    ...
                    <input onChange={(e) => setEmail(e.target.value)} value={email} className='w-full h-11 px-3.5 bg-slate-50/80 border border-slate-200 rounded-xl text-xs font-semibold text-gray-800 placeholder-gray-400 hover:bg-white hover:border-purple-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all shadow-2xs mt-1.5' type="email" required placeholder="name@therapique.com" />
                </div>
                <div className='w-full'>
                ...
                {
                    state === 'Admin'
                        ? <p className='text-xs text-gray-600 mt-1'>Doctor Login? <span onClick={() => setState('Doctor')} className='text-black font-semibold underline cursor-pointer'>Click here</span></p>
                        : <p className='text-xs text-gray-600 mt-1'>Admin Login? <span onClick={() => setState('Admin')} className='text-black font-semibold underline cursor-pointer'>Click here</span></p>
                }
            </div>
        </form>
    )
