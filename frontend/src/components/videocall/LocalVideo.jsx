...
const LocalVideo = ({ stream, isVideoMuted, isAudioMuted, userName }) => {
  ...
  useEffect(() => {
    console.log('[PATIENT LOCAL VIDEO] received stream:', stream)
    console.log('[PATIENT LOCAL VIDEO] stream ID:', stream?.id)
    console.log('[PATIENT LOCAL VIDEO] video tracks:', stream?.getVideoTracks())

    const videoEl = videoRef.current

    if (videoEl && stream && !isVideoMuted) {
      console.log('[PATIENT LOCAL VIDEO] assigning srcObject')
      videoEl.srcObject = stream
      console.log('[PATIENT LOCAL VIDEO] actual srcObject:', videoEl.srcObject)
      videoEl.play().catch(e => console.warn('[PATIENT LOCAL VIDEO] Play warning:', e))
    } else if (videoEl && (!stream || isVideoMuted)) {
      console.log('[PATIENT LOCAL VIDEO] clearing srcObject and pausing')
    ...
    return () => {
      if (videoEl) {
        console.log('[PATIENT LOCAL VIDEO CLEANUP] Pausing video element and clearing srcObject')
        try { videoEl.pause() } catch (e) {}
        videoEl.srcObject = null
        try { videoEl.load() } catch (e) {}
      }
